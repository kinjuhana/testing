...
if ( $_GET['action'] == 'movie' || strposa($uri, options('url_pt') ) ) :
if ( isset($_GET['id'] ) || strposa($uri, options('url_pt') ) ) {
        if(strposa($uri, options('url_pt') ) ) {
                $str = explode("/", $uri);
                if (is_numeric($str[2]) || strposa($str[2], 'tt' ) !== false ) { $TMDBID = $str[2]; }else{ header('Location: /'); }
        }else{ $TMDBID = $_GET['id']; }

	$path	  = $_SERVER['DOCUMENT_ROOT'] . '/cache/pt/';
