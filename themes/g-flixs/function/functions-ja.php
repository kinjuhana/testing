...
function gsilvers_data_movie( $nama = 'home_m_', $page = 1, $get = 'getNowPlayingMovies') {
        $apikey = tmdb_api();
        $tmdb   = new TMDB($apikey, en, true);
	$path	= DOCUMENT_ROOT . '/cache/home/';
	$name	= $nama.$page.'.json';
	if ( file_exists( $path . $name ) && ! gsilvers_expire( $path . $name ) ) {
		...
		if ( $Movie['results'] ) {
                        ...
                        foreach((array)$Movie['results'] as $row) {
                        	...
                        	if ($row[poster_path]) {
                                	...
                                	$item['poster_path'] = '/assets/images/no-cover.png';
                        	}
                        	if ($row[backdrop_path]) {
                                	$item['backdrop_path'] = 'https://image.tmdb.org/t/p/w780'.$row[backdrop_path];
                        	}else{
                                	$item['backdrop_path'] = '/assets/images/no-backdrop.png';
                        	}
...
function gsilvers_data_genre( $nama = 'home_g_', $id = '', $page = 1, $get = 'GetGenreMovies') {
        $apikey = tmdb_api();
        $tmdb   = new TMDB($apikey, en, true);
...
function gsilvers_data_tv( $nama = 'home_tv_', $page = 1, $get = 'getOnTheAirTVShows') {
        $apikey = tmdb_api();
        $tmdb   = new TMDB($apikey, en, true);
...
function gsilvers_data_search_movie( $query = '', $page = 1) {
        $apikey = tmdb_api();
        $tmdb   = new TMDB($apikey, en, true);
...
function gsilvers_data_search_tv( $query = '', $page = 1) {
        $apikey = tmdb_api();
        $tmdb   = new TMDB($apikey, en, true);
...
function gsilvers_search( $query, $nama = 'search_' ) {
	$path	= DOCUMENT_ROOT . '/cache/search/';
	$name	= $nama.'.json';
	if ( file_exists( $path . $name ) && ! gsilvers_expire( $path . $name , 86400 ) ) {
                $data = @file_get_contents( $path . $name );
		return $data;
	} else {
...
if ( $_GET['action'] == 'movie' || strposa($uri, options('url_ja') ) ) :
if ( isset($_GET['id'] ) || strposa($uri, options('url_ja') ) ) {
        if(strposa($uri, options('url_ja') ) ) {
                $str = explode("/", $uri);
                if (is_numeric($str[2]) || strposa($str[2], 'tt' ) !== false ) { $TMDBID = $str[2]; }else{ header('Location: /'); }
        }else{ $TMDBID = $_GET['id']; }

	$path	  = $_SERVER['DOCUMENT_ROOT'] . '/cache/ja/';
	$basename = $TMDBID.'.json';
	if ( file_exists( $path . $basename ) && ! gsilvers_expire( $path . $basename , 86400 ) ) {
                $data = @file_get_contents( $path . $basename );
                $row = unserialize( $data );
	} else {
                $apikey = tmdb_api();
                $tmdb = new TMDB($apikey, 'ja', true);
...
{
        $images = 'https://image.tmdb.org/t/p/w1280' . $row['backdrop_path'];
} elseif ($row['poster_path']!=null){
        $images = 'https://image.tmdb.org/t/p/w1280' . $row['poster_path'];
} else {
                                $images = '/assets/images/no-backdrop.png';
                                $w780 = '/assets/images/no-backdrop.png';
                        }
                        if ($row['poster_path']!=null) {
                                $images_small = 'https://image.tmdb.org/t/p/w300' . $row['poster_path'];
                        } elseif ($row['backdrop_path']!=null){
                                $images_small = 'https://image.tmdb.org/t/p/w300' . $row['backdrop_path'];
                        } else {
                                $images_small = '/assets/images/no-cover.png';
                        }
     	     	        if (is_array($row['genres'])){
             	     	        $genres  = array();foreach($row['genres'] as $result) : $genres[] = '<a title="'.$result['name'].' Movies" href="'.seocat(permalink($result['name']),$result['id']).'">'.$result['name'].'</a>';endforeach;
             	     	        $genre = implode(", ",$genres);
     	     	        }
     	     	        if (is_array($row['genres'])){
             	     	        $genres  = array();foreach($row['genres'] as $result) : $genres[] = '<a title="'.$result['name'].' Movies" href="'.seocat(permalink($result['name']),$result['id']).'">'.$result['name'].'</a>';if ($ic++ == 0) break;endforeach;
             	     	        $jenre = implode(", ",$genres);
     	     	        }
     	     	        if (is_array($row['genres'])){
     	     	        ...
     	     	        {       
             	     	        $ic = 0;$casts = array();foreach($row['credits']['cast'] as $result) :$casts[] = '<a title="'.$result['name'].'">'.$result['name'].'</a>';if ($ic++ == 3) break;endforeach;
             	     	        $cast = implode(", ",$casts);
     	     	        }
     	     	        if (is_array($row['credits']['cast']))
     	     	        {       
             	     	        $ic = 0;$casts = array();foreach($row['credits']['cast'] as $result) :$casts[] = $result['name'];if ($ic++ == 1) break;endforeach;
             	     	        $artis = implode(", ",$casts);
     	     	        }
     	     	        if (is_array($row['credits']['crew']))
     	     	        {       
             	     	        $ic = 0;$crews = array();foreach($row['credits']['crew'] as $result) :$crews[] = '<a title="'.$result['name'].'">'.$result['name'].'</a>';if ($ic++ == 0) break;endforeach;
             	     	        $crew = implode(", ",$crews);
     	     	        }
     	     	        if (is_array($row['credits']['crew']))
     	     	        {       
             	     	        $ic = 0;$crews = array();foreach($row['credits']['crew'] as $result) :$crews[] = $result['name'];if ($ic++ == 0) break;endforeach;
             	     	        $direktur = implode(", ",$crews);
     	     	        }
                ...
                if ( $row['name'] != '' ) {
                        ...
                        if ($row[images]['backdrops']!=null) {
                                ...
                                foreach($row[images]['backdrops'] as $result) {
                                        $images = 'https://image.tmdb.org/t/p/w1280' . $result['file_path'];
                        ...
                        if ($row['poster_path']!=null) {
     	     	        ...
     	     	        if (is_array($row['genres'])){
             	     	        $genres  = array();foreach($row['genres'] as $result) : $genres[] = '<a title="'.$result['name'].'">'.$result['name'].'</a>';endforeach;
             	     	        $genre = implode(", ",$genres);
     	     	        }
     	     	        if (is_array($row['genres'])){
             	     	        foreach($row['genres'] as $result) : $category = $result['name'];$categoryid = $result['id'];endforeach;
     	     	        ...
     	     	        if (is_array($row['credits'][cast]))
     	     	        {       
             	     	        $ic = 0;$casts = array();foreach($row['credits'][cast] as $result) :$casts[] = '<a title="'.$result['name'].'">'.$result['name'].'</a>';if ($ic++ == 10) break;endforeach;
             	     	        $cast = implode(", ",$casts);
     	     	        }
                        if($row['vote_average'] > 0) {
	                        $get_rating =  $row['vote_average'];
                        ...
                        if($str[2] != '') {
                            if($row3[still_path])
                            {
                                $images = 'https://image.tmdb.org/t/p/w1280'.$row3[still_path];
                            }
                        }
                } else {
                        gsilvers_throw();
