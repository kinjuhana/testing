...
if ( $_GET['action'] == 'movie' || strposa($uri, options('url_fr') ) ) :
if ( isset($_GET['id'] ) || strposa($uri, options('url_fr') ) ) {
        if(strposa($uri, options('url_fr') ) ) {
                $str = explode("/", $uri);
                if (is_numeric($str[2]) || strposa($str[2], 'tt' ) !== false ) { $TMDBID = $str[2]; }else{ header('Location: /'); }
        }else{ $TMDBID = $_GET['id']; }

	$path	  = $_SERVER['DOCUMENT_ROOT'] . '/cache/fr/';
